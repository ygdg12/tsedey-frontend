import '../App.css'

// Social Media Icons as SVG Components
const TelegramIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 0C5.373 0 0 5.373 0 12s5.373 12 12 12 12-5.373 12-12S18.627 0 12 0zm5.894 8.221l-1.97 9.28c-.145.658-.537.818-1.084.508l-3-2.21-1.446 1.394c-.14.18-.357.295-.6.295-.002 0-.003 0-.005 0l.213-3.054 5.56-5.022c.24-.213-.054-.334-.373-.12l-6.87 4.326-2.96-.924c-.64-.203-.658-.64.135-.954l11.566-4.458c.538-.196 1.006.128.832.941z"/>
  </svg>
)

const InstagramIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
  </svg>
)

const TikTokIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
  </svg>
)

const YouTubeIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
  </svg>
)

const PhoneIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg">
    <path d="M22 16.92v2a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.12 1h2a2 2 0 0 1 2 1.72 12.85 12.85 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7 9a16 16 0 0 0 6 6l1.36-1.36a2 2 0 0 1 2.11-.45 12.85 12.85 0 0 0 2.81.7A2 2 0 0 1 22 16.92Z" />
  </svg>
)

const MailIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg">
    <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2Z" />
    <path d="m22 6-10 7L2 6" />
  </svg>
)

const PinIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" xmlns="http://www.w3.org/2000/svg">
    <path d="M21 10c0 5.25-6 11-9 11s-9-5.75-9-11a9 9 0 1 1 18 0Z" />
    <circle cx="12" cy="10" r="3" />
  </svg>
)

export default function Contacts() {
  const socialLinks = [
    {
      id: 1,
      name: 'Telegram',
      Icon: TelegramIcon,
      url: 'https://t.me/tsedey_activewear',
      handle: '@tsedey_activewear',
    },
    {
      id: 2,
      name: 'Instagram',
      Icon: InstagramIcon,
      url: 'https://instagram.com/tsedeyactivewear',
      handle: '@tsedeyactivewear',
    },
    {
      id: 3,
      name: 'TikTok',
      Icon: TikTokIcon,
      url: 'https://www.tiktok.com/@tsedey.active',
      handle: '@tsedey.active',
    },
    {
      id: 4,
      name: 'YouTube',
      Icon: YouTubeIcon,
      url: 'https://youtube.com/@tsedeyactivewear',
      handle: '@tsedeyactivewear',
    },
  ]

  return (
    <section className="section contact-page">
      <div className="section-header">
        <h1 className="section-title">Contact</h1>
        <p className="section-subtitle">Reach out for team orders, questions, or availability.</p>
      </div>

      <div className="contact-quick-grid">
        <a className="contact-quick-card" href="tel:+251968888886">
          <div className="contact-quick-icon">
            <PhoneIcon />
          </div>
          <div className="contact-quick-body">
            <div className="contact-quick-title">Call</div>
            <div className="contact-quick-value">+251 96 888 8886</div>
            <div className="contact-quick-note">Fastest way to confirm an order</div>
          </div>
        </a>

        <a
          className="contact-quick-card"
          href="https://t.me/tsedey_activewear"
          target="_blank"
          rel="noopener noreferrer"
        >
          <div className="contact-quick-icon">
            <TelegramIcon />
          </div>
          <div className="contact-quick-body">
            <div className="contact-quick-title">Telegram</div>
            <div className="contact-quick-value">@tsedey_activewear</div>
            <div className="contact-quick-note">Send size list and quantities</div>
          </div>
        </a>

        <a className="contact-quick-card" href="mailto:info@tsedeyactivewear.com">
          <div className="contact-quick-icon">
            <MailIcon />
          </div>
          <div className="contact-quick-body">
            <div className="contact-quick-title">Email</div>
            <div className="contact-quick-value">info@tsedeyactivewear.com</div>
            <div className="contact-quick-note">Best for detailed requests</div>
          </div>
        </a>

        <div className="contact-quick-card contact-quick-card--static">
          <div className="contact-quick-icon">
            <PinIcon />
          </div>
          <div className="contact-quick-body">
            <div className="contact-quick-title">Location</div>
            <div className="contact-quick-value">Addis Ababa, Ethiopia</div>
            <div className="contact-quick-note">Local support & coordination</div>
          </div>
        </div>
      </div>

      <div className="contact-socials">
        <h2 className="contact-socials-title">Follow Tsedey</h2>
        <p className="contact-socials-subtitle">New drops, motion lookbooks, and behind-the-scenes.</p>
        <div className="contact-social-grid">
          {socialLinks.map((social) => {
            const IconComponent = social.Icon
            return (
              <a
                key={social.id}
                href={social.url}
                target="_blank"
                rel="noopener noreferrer"
                className="contact-social-card"
              >
                <span className="contact-social-icon">
                  <IconComponent />
                </span>
                <span className="contact-social-name">{social.name}</span>
                <span className="contact-social-handle">{social.handle}</span>
              </a>
            )
          })}
        </div>
      </div>
    </section>
  )
}

